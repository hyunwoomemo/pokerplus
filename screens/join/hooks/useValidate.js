...
export const useValidate = (type, text, values, error, setError) => {
  switch (type) {
    case "id":
      const regEmail = /^[a-zA-Z0-9+-\_.]+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+$/;
      if (!regEmail.test(text) && text.length > 0) {
        LayoutAnimation.configureNext(LayoutAnimation.Presets.spring);
        setError({ ...error, id: "올바른 이메일 형식이 아닙니다." });
      } else {
        LayoutAnimation.configureNext(LayoutAnimation.Presets.spring);
    ...
    case "password2":
      ...
      break;
    case "engFirst":
      const engFirstPattern = /^[a-zA-Z\s]+$/;
      if (!engFirstPattern.test(text) && text.length > 0) {
        LayoutAnimation.configureNext(LayoutAnimation.Presets.spring);
      ...
      break;
    case "engLast":
      const engLastPattern = /^[a-zA-Z\s]+$/;
      if (!engLastPattern.test(text) && text.length > 0) {
        LayoutAnimation.configureNext(LayoutAnimation.Presets.spring);
